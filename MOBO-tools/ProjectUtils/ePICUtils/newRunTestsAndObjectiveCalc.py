...
class SubJobManager:
    ...
    def makeSlurmScript_data_production(self, particle):
        ...
        with open(filename,"w") as file:
            ...
            file.write(f"source {workEicPath}/setup.sh\n")
            file.write(f"python3 {workEicPath}/slurm/submit_workflow.py --compactFile {compactFileName} --setupPath {setupPath} --loadEpicPath {loadEpicPath} --run_name_pref {run_name_pref} --runNum {self.job_id} --chPath {MOBO_path} --particle {particle} --skipTraining --no-classification --geo_config linear_ratio")
        return filename
