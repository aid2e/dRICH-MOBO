...
class SubJobManager:
    ...
    def retrieveResults(self):
        ...
        if len(roc_scores) == len(self.p_points):
            final_results = np.array([roc_scores[0], roc_scores[1], self.outer_radius])
        else:
    ...
    def calcGeomVals(self):
        ...
        self.superlayer_dist = self.steel_thick + self.sens_sublayer_thick * 2 + self.air_gap_thick * 4 # width of superlayer
        ...
        self.adj_sens_sublayer_dist = self.sens_sublayer_thick + self.air_gap_thick * 2 # distance between adjacent sensitive sublayers within the same superlayer
        ...
        self.layer_pos[::2] = [self.first_sens_sublayer_pos + self.superlayer_dist*i for i in range(self.superlayer_count)] # first sublayers 
        self.layer_pos[1::2] = self.layer_pos[::2] + self.adj_sens_sublayer_dist # second sublayers
    ...
    def layer_num(self, xpos, ypos):
        ...
        in_second_layer = np.logical_and(within_layer_region, np.logical_and(dis_from_first_sublayer * 1.0001 >= self.adj_sens_sublayer_dist, dis_from_first_sublayer / 1.0001 <= self.adj_sens_sublayer_dist + self.sens_sublayer_thick))
    ...
    def layer_calc(self, xpos, ypos, zpos, energy_dep):
        ...
        layer_counts = np.asarray(ak.sum(layers_traveled[:, None] == np.arange(1, self.superlayer_count * 2 + 1), axis=0)) # find counts for each layer, 1 through max
