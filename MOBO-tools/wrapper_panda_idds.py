...
from ProjectUtils.panda_idds_utilities_multi_steps import (
    build_experiment_pandaidds as build_experiment_pandaidds_mul,
    PanDAIDDSJobRunner as PanDAIDDSJobRunner_mul,
    PanDAIDDSJobMetric as PanDAIDDSJobMetric_mul,
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--name", help="workflow name", type=str, default="drich-mobo"
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    names = ["piKsep_etalow", "piKsep_etahigh", "acceptance"]
    metrics = []
    ...
    mo = MultiObjective(objectives=[Objective(m) for m in metrics])
    ...
    optimization_config = MultiObjectiveOptimizationConfig(
        objective=mo, objective_thresholds=objective_thresholds
    )
    ...
    if args.backend == "slurm":
        ...
        bundle = RegistryBundle(metric_clss={PanDAIDDSJobMetric: None}, runner_clss={PanDAIDDSJobRunner: None})
    elif args.backend == "panda_multi_steps":
        ...
        bundle = RegistryBundle(metric_clss={LocalJobMetric: None}, runner_clss={LocalJobRunner: None})

    # set up sql storage
    db_settings = DBSettings(
        url="sqlite:///{}.db".format(outname), encoder=bundle.encoder, decoder=bundle.decoder
    )
    ...
    try:
        experiment = _load_experiment(experiment_name=experiment_name, decoder=bundle.decoder)  # This uses the full lambda setup
    ...
    for trial in experiment.trials.values():
        ...
        print(
            f"Trial {trial.index}: status={trial.status.name}, generation_method={getattr(trial, 'generation_method', 'N/A')}"
        )
    ...
    if resume_experiment:
        ...
        print("Tot Existing ", n_existing)
        n_to_run = N_TOTAL - n_existing
        print("n_to_run ", n_to_run)
        try:
            if n_to_run > 0:
                print("Resumed experiment: before run_n_trials")
                scheduler.run_n_trials(max_trials=n_to_run)
        ...
        print("Resumed experiment: after run_n_trials")

    else:
        print("Current Experiment: before run_n_trials")
        scheduler.run_n_trials(max_trials=N_TOTAL)
        print("Current Experiment: after run_n_trials")
        model_obj = Models.BOTORCH_MODULAR(experiment=experiment, data=experiment.fetch_data())
        hv = observed_hypervolume(modelbridge=model_obj)
        print(f"hv: {hv}")
